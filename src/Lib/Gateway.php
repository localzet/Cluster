...
class Gateway
{
    ...
    public static function sendToAll($message, $client_id_array = null, $exclude_client_id = null, $raw = false)
    {
        ...
        if (static::$businessServer) {
            foreach (static::$businessServer->gatewayConnections as $address => $gateway_connection) {
    ...
    protected static function getAllGatewayAddress()
    {
        if (isset(static::$businessServer)) {
            ...
            if (empty($addresses)) {
                throw new Exception('Метод businessServer::getAllGatewayAddresses ничего не вернул');
            }
        } else {
    ...
    public static function closeCurrentClient($message = null)
    {
        if (!Context::$connection_id) {
            throw new Exception('Метод closeCurrentClient не может быть вызван в асинхронном контексте');
        }
    ...
    {
        if (!Context::$connection_id) {
            throw new Exception('Метод destoryCurrentClient не может быть вызван в асинхронном контексте');
        }
        $address = long2ip(Context::$local_ip) . ':' . Context::$local_port;
    ...
    protected static function sendAndRecv($address, $data)
    {
        ...
        if (strlen($buffer) === stream_socket_sendto($client, $buffer)) {
            ...
            throw new Exception("Ошибка sendAndRecv($address, \$bufer)! Не могу отправить данные!", 502);
        }
    }
    ...
    {
        // 有$businessServer说明是workerman环境，使用$businessServer发送数据
        if (static::$businessServer) {
            if (!isset(static::$businessServer->gatewayConnections[$address])) {
                return false;
            }
            return static::$businessServer->gatewayConnections[$address]->send($gateway_buffer, true);
        }
        // 非workerman环境
        $gateway_buffer = static::$secretKey ? static::generateAuthBuffer() . $gateway_buffer : $gateway_buffer;
        $client = static::getGatewayConnection("tcp://$address");
    ...
    protected static function getAllGatewayAddressesFromRegister()
    {
        ...
        if (empty($addresses_cache) || $time_now - $last_update > $expiration_time) {
            ...
            if (!$client) {
                throw new Exception('Не могу подключиться к tcp://' . $register_address . ' ' . $errmsg);
            }

            fwrite($client, '{"event":"server_connect","secret_key":"' . static::$secretKey . '"}' . "\n");
            stream_set_timeout($client, 5);
            $ret = fgets($client, 655350);
            if (!$ret || !$data = json_decode(trim($ret), true)) {
                throw new Exception('Ошибка getAllGatewayAddressesFromRegister. tcp://' .
        ...
        if (!$addresses_cache) {
            throw new Exception('Gateway::getAllGatewayAddressesFromRegister() с registerAddress:' .
                json_encode(static::$registerAddress) . '  вернул ' . var_export($addresses_cache, true));
        }
    ...
    {
        if (empty($group)) {
            echo new \Exception('Группа (' . var_export($group, true) . ') пуста');
            return false;
        }
        return true;
    }
    ...
    protected static function getGatewayConnection($address)
    {
        ...
        if (!isset(static::$gatewayConnections[$address])) {
            ...
            if (!$client) {
                throw new Exception("Не могу подключиться к $address $errmsg");
            }
